...
---Additional Rules---
- Ignore metadata such as "依据", "知识库ID", "IDXX", "来源编号" or any similar source references. These are not entities or relationships.
- Only extract meaningful entities or real-world relationships. Do not include database references, lookup codes, or citation IDs as entities.
...
- 忽略礼貌用语和虚词结构词,不提取“请问”、“你好”、“谢谢”、“是否”、“能否”、“有没有”等无实际语义的词汇。
- 避免主观性或程度词,不要提取如“最好的”、“非常严重”、“极其重要”等主观或情绪化词汇。
- 去除语义重复或同义词,比如多个表达相似含义的词汇中只保留一个，避免冗余。错误示例：["自然灾害", "灾难", "灾害"],正确示例应该改为：["自然灾害"]
- 如果查询属于特定任务领域（如灾害响应），则即使用户未明确提及，也应主动补全如具体的灾害类型或者受灾区域等必然相关的关键词。
...
#############################""",
    """Example 4（避免过细地名和无用礼貌词）:

Query: "请问您能帮我分析一下杭州市西湖区文三路XXX大厦附近的暴雨预警情况吗？谢谢！"
################
Output:
{
  "high_level_keywords": ["暴雨预警", "气象风险", "杭州市"],
  "low_level_keywords": ["西湖区", "暴雨", "强降水"]
}
#############################
说明：
- 忽略无意义的修饰词或礼貌语（如“请问”、“你能帮我”、“谢谢”等）；
- 忽略过于细化的地址信息（如“xx路xx号大厦”,"某个大学的名称"，“具体的某个社区名”），仅保留主要城市或区域名；
- 不要提取主观程度词（如“最好的”、“非常重要的”）；
""",
    """Example 5（避免重复近义词，限制关键词数量）:

Query: "在未来5到10年内，中国的新能源汽车发展趋势会如何？尤其是在电池技术、充电基础设施、政策扶持等方面会有哪些突破？"

################
Output:
{
  "high_level_keywords": ["新能源汽车", "发展趋势", "技术突破", "中国"],
  "low_level_keywords": ["电池技术", "充电设施", "政策扶持", "未来5到10年"]
}
#############################
说明：
- 没有重复列出“电动车”、“电动汽车”、“新能源车”等语义重复词
- 控制关键词总数（各类不超过5个）
- 保留核心问题维度：技术、政策、时间段
""",
    """Example 6:

Query: "我们需要尽快安排救援力量，天气预报说后天有暴雨"

################
Output:
{
  "high_level_keywords": ["灾害响应", "救援调度", "气象灾害"],
  "low_level_keywords": ["暴雨", "天气预报", "受灾区域", "灾害类型"]
}
#############################""",
]
...
You are a helpful assistant and a senior emergency response expert tasked with generating a complete and actionable emergency plan based on structured disaster-related document chunks provided in JSON format.
